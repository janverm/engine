...
import (
	...
	"github.com/g3n/engine/audio/oggvorbis"
)
...
type AudioFile struct {
	...
	ewavef  *fs.File          // Pointer to embedded wave file opened filed (nil for vorbis)
	vorbisf *oggvorbis.OVFile // Pointer to vorbis file structure (nil for wave)
...
func NewAudioFile(filename string, efs *embed.FS) (*AudioFile, error) {
	...
	if efs == nil {
		...
		// Try to open as a wave file
		if af.openEmbedWave(filename, efs) == nil {
			return af, nil
		}

		// Try to open as an ogg vorbis file
		if af.openEmbedVorbis(filename, efs) == nil {
			return af, nil
		}
	}
...
func (af *AudioFile) Close() error {
	...
	return oggvorbis.Clear(af.vorbisf)
}
...
func (af *AudioFile) Read(pdata unsafe.Pointer, nbytes int) (int, error) {
	...
	for decoded < nbytes {
		...
		if n == 0 {
			...
			err = oggvorbis.PcmSeek(af.vorbisf, 0)
			if err != nil {
				return 0, err
			}
		}
...
func (af *AudioFile) Seek(pos uint) error {
	...
	return oggvorbis.PcmSeek(af.vorbisf, int64(pos))
}
...
func (af *AudioFile) CurrentTime() float64 {
	...
	if af.vorbisf != nil {
		pos, _ := oggvorbis.TimeTell(af.vorbisf)
		return pos
	}
...
// openWave tries to open the specified file as a wave file
// and if succesfull, sets the file pointer positioned after the header.
func (af *AudioFile) openEmbedWave(filename string, efs *embed.FS) error {

	// Open file
	osf, err := efs.Open(filename)
	if err != nil {
		return err
	}
	...
	// Seeks after the header
	seeker := osf.(io.Seeker)
	_, err = seeker.Seek(waveHeaderSize, 0)
	if err != nil {
		osf.Close()
		return err
	}
...
func (af *AudioFile) openVorbis(filename string) error {
	...
	timeTotal, err := oggvorbis.TimeTotal(vf, -1)
	if err != nil {
		oggvorbis.Clear(vf)
		return nil
	}
